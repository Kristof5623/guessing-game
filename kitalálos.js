...
tippBtn.addEventListener("click", function(){
    ...
    if (rnum > guess){
        ...
        result.textContent = `Gratulálok ${username1}, eltaláltad ${maradt}. próbából! :) (A szám, amire gondoltam: ${rnum})`;
        roundsp1++;
        p1point++;
    ...
    if (attempts === 0){
        setTimeout(() => {
            result.textContent = `Sajnos elfogytak a tippelési lehetőségeid ${username1} ): , amúgy a(z) ${rnum}-re/ra gondoltam :)`;
            roundsp1++;
            tippLines.classList.add("hidden");
            result.classList.remove("hidden");
            nextBtn.classList.remove("hidden");
...
function resetGame(){
    ...
    if (clickCount % 2 === 0) {
        player1Corner.innerHTML = username1; // 1., 3., 5. kör (ha 0-ról indul az 1. kör)
    } else {
        player1Corner.innerHTML = username2; // 2., 4., 6. kör
    }
}
...
if (maxrounds == roundsp1){
    nextBtn.addEventListener("click" , function(){
    resetGame();
    if (maxrounds == roundsp2){
        nextBtn.classList.add("hidden");
